...
p1 <- p1 +
  ...
  geom_path(data = plot_t_extreme, aes(x = min, y = h, color = "Spannweite")) + #Min
  geom_path(data = plot_t_extreme, aes(x = max, y = h), color = "red") + #Max
  geom_path(data = plot_t_extreme, aes(x = mean, y = h, color = "Mittelwert")) #Mean
...
mean_t["a"] <- vert_t_mean$a
mean_t["b"] <- vert_t_mean$b
mean_t["c"] <- vert_t_mean$c
mean_t["d"] <- vert_t_mean$d

#Humidity
mean_lfk <- data.frame(1)
mean_lfk["Grösse"] <- c("Luftfeuchtigkeit")
mean_lfk["a"] <- vert_lfk_mean$a
mean_lfk["b"] <- vert_lfk_mean$b
mean_lfk["c"] <- vert_lfk_mean$c
mean_lfk["d"] <- vert_lfk_mean$d
